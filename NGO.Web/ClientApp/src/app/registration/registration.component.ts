...
import { CityInterface, CountryInterface, StateInterface } from '../userprofile/models/UpdateUserModel';
...
export class RegistrationComponent implements OnInit {
  ...
  spouseCountries: CountryInterface[] = [];
  spouseStates: StateInterface[] = []
  spouseCities: CityInterface[] = [];
  childCountries: CountryInterface[] = [];
  childStates: StateInterface[] = []
  ...
  composeModel(): void {
    ...
    this.registrationModel.city = this.signUpForm.value.city.name;
    ...
    //this.registrationModel.password = this.signUpForm.value.password;
    //Spouse Details
    ...
    this.spouseDetails.spouseCity = this.signUpForm.value.spouseCity.name;
  ...
  GetAllCoutries() {
    this.registrationService.getAllCountries().then(data => {
      debugger
      this.countries = data;
      this.spouseCountries = data;
      this.childCountries = data;
  ...
  onChangeSpouseCountry(e: any) {
    debugger
    if (e.value) {
      // var res=this.spouseCountries.filter(x=>x.id==e.target.value);
      // var countryName=res.find(x=>x.id==e.target.value)?.name;
      // this.spouseCountryName=countryName?countryName:"";
      //   res.forEach(ele => {
      //    ele.states.forEach(element => {
      //      this.spouseStates.push(element);
      //    });
      //  });
      this.spouseStates = e.value.states
    } else {
      this.spouseStates = [];
      this.spouseCities = [];
    }
  ...
  onChangeSpouseState(e: any) {
    debugger
    if (e.value) {
      //  var res=this.spouseStates.filter(x=>x.id==e.target.value);
      //  var stateName=res.find(x=>x.id==e.target.value)?.name;
      //  this.spousestateName=stateName?stateName:"";
      //  res.forEach(ele => {
      //   ele.cities.forEach(element => {
      //     this.spouseCities.push(element);
      //   });
      // });
      this.spouseCities = e.value.cities;
    } else {
      this.spouseCities = [];
    }
  }
  onChangeChildCountry(e: any) {
    ...
    if (e.value) {
      //   var res=this.spouseCountries.filter(x=>x.id==e.target.value);
      //   var countryName=res.find(x=>x.id==e.target.value)?.name;
      //   this.childcountryName=countryName?countryName:"";
      //   res.forEach(ele => {
      //    ele.states.forEach(element => {
      //      this.childStates.push(element);
      //    });
      //  });
      this.childCountryName = e.value;
      this.childStates = e.value.states
    } else {
      this.childStates = [];
      this.childCities = [];
    }
  ...
  onChangeChildState(e: any) {
    debugger
    if (e.value) {
      //  var res=this.childStates.filter(x=>x.id==e.target.value);
      //  var stateName=res.find(x=>x.id==e.target.value)?.name;
      //  this.childstateName=stateName?stateName:"";
      //  res.forEach(ele => {
      //   ele.cities.forEach(element => {
      //     this.childCities.push(element);
      //   });
      // });
      this.childStateName = e.value
      this.childCities = e.value.cities
    } else {
      this.childCities = [];
    }
  }
